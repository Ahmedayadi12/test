...
function addToCart(event){
    ...
    <td><button class="ubutton uk-button-danger" type="button">Remove</button></td>
`
...
function totalCost(event){
    ...
    price_field_content = price_field.innerText.replace('dt','')
...
function grandTotal(){
    ...
    for(let i = 0; i < all_total_fields.length; i++){
        all_prices = Number(all_total_fields[i].innerText.replace('dt', ''))
        total+=all_prices
    }
    grand_total.children[0].innerText = "$"+parseInt(total)
